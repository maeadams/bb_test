...
resource "azurerm_key_vault" "kv" {
  for_each = var.key_vault_config != null ? { "key_vault" = var.key_vault_config } : {} 
...
resource "azurerm_key_vault_key" "keys" {
  for_each = var.key_vault_config != null && var.encryption_keys != null ? var.encryption_keys : {}
...
resource "azurerm_key_vault_secret" "secrets" {
  for_each = var.key_vault_config != null && var.key_vault_secrets != null ? var.key_vault_secrets : {}
  name         = each.value.name
...
#   triggers = {
#     key_vault_name = azurerm_key_vault.kv[each.key].name
#     resource_group = var.resource_group_name
#     tenant_id = data.azurerm_client_config.current.tenant_id
#   }
...
resource "azurerm_application_gateway" "app_gw" {
  ...
  dynamic "gateway_ip_configuration" {
    ...
    content {
      ...
      subnet_id = data.azurerm_subnet.app_gateway_subnet[count.index].id #lookup(gateway_ip_configuration.value.subnet)#var.subnet_ids[gateway_ip_configuration.value.subnet_id]  # Use variable lookup
    }
  }
