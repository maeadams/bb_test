...
data "azurerm_subnet" "subnet" {
  ...
  name                 = each.value.name                 # The actual subnet name
  virtual_network_name = each.value.virtual_network_name  # The VNet name
  resource_group_name  = each.value.resource_group_name   # The resource group name
}
  
resource "azurerm_network_interface" "vm" {
  for_each = var.vms
  name                = "nic-vic-${var.vms[each.key].name}"
...
# resource "random_password" "PwGenerator" {
#   count = var.vm_number
#   length           = 16
#   special          = true
#   override_special = "_%@"
...
#   count = var.vm_number
#   name         = "secret-vic-${var.environment}-${var.resource_region}-${var.application_code}-${format("%03d", count.index+1)}"
#   value        = random_password.PwGenerator[count.index].result
#   key_vault_id = data.azurerm_key_vault.kv.id
#   depends_on = [ random_password.PwGenerator ]
# }
resource "azurerm_virtual_machine" "vm" {
  ...
  storage_os_disk {
    name              = "sod-vic-${var.vms[each.key].name}"
   ...
   storage_data_disk {
    name              = "sdd-vic-${var.vms[each.key].name}"
    create_option     = each.value.create_option
    lun               = 1
    disk_size_gb      = each.value.disk_size_gb
  }
  ...
  os_profile {
    computer_name  = "vic${var.vms[each.key].name}"
    admin_username = "admin"
    admin_password = each.value.admin_password # Use the Key Vault secret for the password
  }
  # Configuration for Linux
  dynamic "os_profile_linux_config" {
    for_each = var.vms[each.key].os_type == "Linux" ? [1] : []
    content {
  ...
  dynamic "os_profile_windows_config" {
    for_each = var.vms[each.key].os_type == "Windows" ? [1] : []
    content {
...
data "azurerm_virtual_machine" "vm_linux" {
  for_each = var.vms != null && var.vms.key.os_type == "Linux" ? var.vms : {}
  name                = each.value.name
  resource_group_name = var.resource_group_name
  depends_on = [ azurerm_virtual_machine.vm ]
}
data "azurerm_virtual_machine" "vm_windows" {
  for_each = var.vms != null && var.vms.key.os_type == "Windows" ? var.vms : {}
  name                = each.value.name
  resource_group_name = var.resource_group_name
  depends_on = [ azurerm_virtual_machine.vm ]
}
resource "azurerm_virtual_machine_extension" "aad_login_linux" {
  for_each = var.vms != null && var.vms.key.os_type == "Linux" ? var.vms : {}
  name                 = "vm-extension-linux-ADDLogin-${var.vms[each.key].name}"
  virtual_machine_id   = data.azurerm_virtual_machine.vm_linux[each.key].id
...
resource "azurerm_virtual_machine_extension" "aad_login_windows" {
  for_each = var.vms != null && var.vms.key.os_type == "Windows" ? var.vms : {}
  name                 = "vm-extension-windows-ADDLogin-${var.vms[each.key].name}"
  virtual_machine_id   = data.azurerm_virtual_machine.vm_windows[each.key].id
  publisher            = "Microsoft.Azure.ActiveDirectory"
  type                 = "AADSSHLoginForWindows" 
  type_handler_version = "1.0"
  depends_on = [ azurerm_virtual_machine.vm ] 
}
# resource "azurerm_role_assignment" "kv" {
#   count = (var.vm_contributor_object_id != null && var.kv_secret_user_role_assignement > 0) ? var.kv_secret_user_role_assignement : 0
#   scope                = data.azurerm_key_vault.kv.id
#   role_definition_name = "Key Vault Secrets User"
#   principal_id         = var.vm_contributor_object_id
# }

# resource "azurerm_role_assignment" "vm_contributor_role" {
#   count = (var.vm_contributor_object_id != null && var.vm_contributor_role_assignement > 0) ? var.vm_contributor_role_assignement : 0
#   scope                = "${element(azurerm_virtual_machine.vm.*.id, count.index)}"
...
#   for_each = var.enable_autoshutdown != null && var.vms != null ? var.enable_autoshutdown : {}
